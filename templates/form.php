...
?><form action="<?php print_unescaped(\OC::$server->getURLGenerator()->linkToRoute('registration.register.validateEmail')) ?>" method="post">
	...
	<fieldset>
		...
		<?php } ?>
		<p class="grouptop">
			<input type="email" name="email" id="email" value="<?php echo !empty($_['entered_data']['user']) ? $_['entered_data']['email'] : ''; ?>" placeholder="<?php p($l->t('Email')); ?>" />
		...
		<input type="submit" id="submit" value="<?php p($l->t('Sign up')); ?>" />
	</fieldset>
</form>
